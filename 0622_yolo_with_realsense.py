        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    mid_pos = get_mid_pos(xyxy, depth_frame, profile.get_stream(rs.stream.color).as_video_stream_profile().get_intrinsics())
                    # print(mid_pos[2] * 1000)
                    c = int(cls)  # integer class
                    label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {mid_pos[2] * 1000:.2f}')
