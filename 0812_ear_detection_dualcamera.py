        ...
        try:
            if multi_face_landmarks:
                ...
                for i in range(468):
                    ...
                    pixel_x = int(pixel_point.x * self.color_image.shape[1])
                    pixel_y = int(pixel_point.y * self.color_image.shape[0])
...
class PipelineProcess(Thread):
    def __init__(self, ser, device: RealSenseCamera):
        Thread.__init__(self)
        self.serial = ser
        self.device = device
        self.ret = None
    ...
    def run(self):
        ...
        mp_drawing_th = mp.solutions.drawing_utils
        mp_face_mesh_th = mp.solutions.face_mesh

        with mp_face_mesh_th.FaceMesh(
                static_image_mode=False,
                min_detection_confidence=0.5,
                min_tracking_confidence=0.5) as face_mesh_th:
            try:
                while True:
                    reset = False
                    ears = np.zeros((0, 3))
                    x_reshaped = np.zeros((468, 3))
                    self.device.get_data()

                    frameset = self.device.frameset
                    self.device.get_aligned_frames(frameset, aligned_to_color=True)

                    frameset = self.device.depth_to_disparity.process(self.device.frameset).as_frameset()
                    frameset = self.device.spatial_filter.process(frameset).as_frameset()
                    frameset = self.device.temporal_filter.process(frameset).as_frameset()
                    frameset = self.device.disparity_to_depth.process(frameset).as_frameset()
                    ...
                    img_rs_th = self.device.color_image.copy()

                    p = LandmarkMaker(img_rs_th, face_mesh_th, self.device.depth_frame, self.device.color_intrinsics)
                    p.start()
                    ...
                    try:
                        ...
                        if not p.ret:
                            _ = q.get()
                            q_main.put([img_rs_th, False, ears, x_reshaped])
                            continue
                    ...
                    if len(face_points) == 1404:
                        ...
                        if plane_norm_unit_vec[2] >= 0:
                            ...
                            s = np.stack([letterbox(x, imgsz, stride=stride)[0].shape for x in img_rs_th], 0)  # shapes
                            ...
                            # Letterbox
                            img_raw = img_rs_th.copy()
                            img_rs_th = img_rs_th[np.newaxis, :, :, :]

                            # Stack
                            img_rs_th = np.stack(img_rs_th, 0)

                            # Convert
                            img_rs_th = img_rs_th[..., ::-1].transpose((0, 3, 1, 2))  # BGR to RGB, BHWC to BCHW
                            img_rs_th = np.ascontiguousarray(img_rs_th)

                            img_rs_th = torch.from_numpy(img_rs_th).to(device_type)
                            img_rs_th = img_rs_th.half() if half else img_rs_th.float()  # uint8 to fp16/32
                            img_rs_th /= 255.0  # 0 - 255 to 0.0 - 1.0
                            if img_rs_th.ndimension() == 3:
                                img_rs_th = img_rs_th.unsqueeze(0)

                            # Inference
                            t1 = time_sync()
                            pred = model(img_rs_th, augment=augment)[0]
                            ...
                            for i, det in enumerate(pred):  # detections per image
                                s = f'{i}: '
                                s += '%gx%g ' % img_rs_th.shape[2:]  # print string
                                if len(det):
                                    # Rescale boxes from img_size to im0 size
                                    det[:, :4] = scale_coords(img_rs_th.shape[2:], det[:, :4], img_raw.shape).round()
                                    ...
                                    for *xyxy, conf, cls in reversed(det):
                                        mid_pos = get_mid_pos(xyxy, self.device.depth_frame,
                        ...
                        q_main.put([img_rs_th, direction, ears, x_reshaped])
...
def run():
    # front_calibration_path = "transform.npz"
    #
    # with np.load(front_calibration_path) as Y:
    #     rot_realsense, trans_realsense, _, _ = [Y[i] for i in ('rot', 'trans', 'ess', 'fund')]
    ...
    serial, device = cameras.popitem()

    master_th = PipelineProcess(serial, device)
    master_th.start()
    ...
    while True:
        ...
        try:
            ...
            if not ret[1]:
                ...
                resized_image = cv2.resize(ret[0], dsize=(0, 0), fx=1, fy=1,
        ...
        img_circle = cv2.circle(ret[0].copy(), list(map(int, swab_point_0)), 2, (255, 0, 0), -1)

        img_disp = cv2.line(img_circle.copy(), list(map(int, swab_point_0)), list(map(int, swab_point_1)),
