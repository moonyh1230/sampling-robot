        ...
        try:
            while True:
                ...
                if len(face_points) == 1404:
                    ...
                    vec_no_93 = x_reshaped[93]
                    vec_no_323 = x_reshaped[323]
                    vec_no_4 = x_reshaped[4]
                    vec_no_6 = x_reshaped[6]
                    ...
                    if plane_norm_unit_vec[2] <= 0:
                        ...
                        swab_vec_old = (nose_point - vec_no_323) / np.linalg.norm(nose_point - vec_no_323)
                        ...
                        proj_norm_vec_old = np.dot(swab_vec_old, plane_norm_unit_vec) * plane_norm_unit_vec
                        proj_vec_old = (swab_vec - proj_norm_vec_old) / np.linalg.norm(swab_vec - proj_norm_vec_old)

                        swab_visualize = nose_point + proj_vec * 100
                        swab_visualize_old = nose_point + proj_vec_old * 100
                        ...
                        swab_point_2 = rs.rs2_project_point_to_pixel(device.color_intrinsics, swab_visualize_old)

                    else:
                        nose_point = np.mean([x_reshaped[20], x_reshaped[60], x_reshaped[75],
                                              x_reshaped[79], x_reshaped[166], x_reshaped[238]],
                                             axis=0)
                        # nose_point = x_reshaped[20]

                        swab_vec = (nose_point - vec_ear) / np.linalg.norm(nose_point - vec_ear)
                        swab_vec_old = (nose_point - vec_no_323) / np.linalg.norm(nose_point - vec_no_93)
                    ...
                    img_disp_old = cv2.line(img_circle.copy(), list(map(int, swab_point_0)), list(map(int, swab_point_2)),
                                            color=(255, 0, 0), thickness=2)

                    resized_image = cv2.resize(img_disp, dsize=(0, 0), fx=1, fy=1,
                                               interpolation=cv2.INTER_AREA)

                    resized_image_old = cv2.resize(img_disp_old, dsize=(0, 0), fx=1, fy=1,
                                                   interpolation=cv2.INTER_AREA)
                ...
                # Show images from both cameras
                cv2.namedWindow('RealSense_front', cv2.WINDOW_NORMAL)
                cv2.namedWindow('without yolo', cv2.WINDOW_NORMAL)
                cv2.resizeWindow('RealSense_front', resized_image.shape[1], resized_image.shape[0])
                cv2.resizeWindow('without yolo', resized_image_old.shape[1], resized_image_old.shape[0])
                cv2.imshow('RealSense_front', resized_image)
                cv2.imshow('without yolo', resized_image_old)
